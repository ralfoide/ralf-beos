...
//***********************************************************
{
	...
	BDirectory dir(gApplication->getAppDir(), K_IMG_DIR);
	...
	for(long i=0; i<6; i++)
	{
		infolabelLen[i]=mNameFont.StringWidth(infolabel[i]);
...
*/
	if (mFilter)
	{
		...
		if (!sc) sc="";
		if (mFilter->sFilter.supportedMode == B_COLOR_8_BIT)
			strcat(so, "8 bpp");
		else if (mFilter->sFilter.supportedMode == B_RGB_16_BIT)
			strcat(so, "16 bpp");
		else if (mFilter->sFilter.supportedMode == (color_space)(B_COLOR_8_BIT | B_RGB_16_BIT))
	...
	if (Window() && Window()->Lock())
	{
		...
		if (filter && mFilter != filter && filter->sFilter.hasSettings)
		{
			...
			if (mFilterView)
			{
				...
				int32 x = mFilterRect.Width();
				int32 y = mFilterRect.Height();
				if (x < referenceBounds.Width()) x = referenceBounds.Width();
				if (y < referenceBounds.Height()) y = referenceBounds.Height();
